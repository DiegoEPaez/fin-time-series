...
def get_series(info_dict):
    ...
    for series, values in info_dict.items():
        if values['source'] == 'YAHOO':
            df = get_data_yahoo(series, start_dt, end_dt, values['yahoo_ticker'], values.get('col', 'Adj Close'))
...
def build_usdmxn(info_dict):
    ...
    # Information for cuenta_capital, cuenta_corriente is only available since 2002-03-31, disregarding that
    # information and training since 1995 has worse performance
    ...
    if len(vars_required) > len(set(vars_required) & set(join.columns)):
        logging.info("Unable to calculate difference of rates and inflation due to missing columns")
        return join
    ...
    join['ppp_value'] = fact_ppp_0203 * (1 + join['inf_inc']) / (1 + join['usdmxn_inc']) - 1
    join['dif_rates'] = ((1 + join['CETES28'] / 100) / (1 + join['FEDFUNDS'] / 100) - 1) * 100
...
def create_ts(data, target, pred_interval, n_steps, ordered=False):
    ...
    for col in data:
        ...
        if 'transform' in info_dict[col]:
            ...
            if np.isnan(data[col]).any():
                logging.warning(f'Nans detected in column {col} when applying transformation {info_dict[col]["transform"]}')
    ...
    if no_ex < 0:
        logging.warning(f"Cannot create examples with given time series, there are: {values.shape[0]} values,and"
