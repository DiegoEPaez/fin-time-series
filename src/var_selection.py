...
from src.pred import errors_dist, split_sets, create_data
...
def error(params, df_symbol, path):
    ...
    errors = errors_dist(
        X, y, symb_model, cv_loops, cv_splits, n_epochs, symb, interval, train_all=True
    )
...
def main():
    ...
    for i in intervals:
        ...
        for k in range(tries_per_interval):
            ...
            print(
                f"Try {k} of {tries_per_interval} for interval {i}, RMSE of {selected_cols}: {rmse}"
            )
            if rmse < best_rmse:
                best_rmse = rmse
                vars = selected_cols
